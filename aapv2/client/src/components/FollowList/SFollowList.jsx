...
import FP from "../FollowList/SFollowPreview"


const SFollowList = props => {
    ...
    useEffect(() => {
        axios.get(`http://localhost:8000/api/user/find/shelters/follow`)
            .then((res) => {
                setFList(res.data);
        });
    }, [props]);
    ...
    const setProView = () =>{
        ...
        axios.get("http://localhost:8000/api/profile/find/shelter",{params:{id: profileID}} )
            .then(res => {
                setProfile(res.data)
                setRight(true)
